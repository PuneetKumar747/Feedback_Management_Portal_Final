...
db_config = {
    'dbname': os.getenv('DB_NAME', 'feedbackportal'),
    'user': os.getenv('DB_USER', 'su23003'),
    'host': os.getenv('DB_HOST', 'dpg-crhh7qt6l47c73c8vjc0-a'),
    'password': os.getenv('DB_PASSWORD', 'BodGhLuRQwZdM9yWHDSk6zmN7bWMoL4N'),
    ...
    try:
        with conn.cursor() as cursor:
            ...
            CREATE TABLE IF NOT EXISTS feedback (
                ...
                remarks TEXT,
                UNIQUE (course_id, student_email, date_of_feedback)
            );
            """)
            ...
            SELECT c.course_name, i.instructor_id, c.batch_pattern
            FROM (VALUES
            ...
            ) AS c(course_name, instructor_email, batch_pattern)
            JOIN instructors i ON c.instructor_email = i.instructor_email
...
def authorize():
    ...
    if user_info:
        ...
        elif re.match(r'^(kpuneet474@gmail\.com|[a-zA-Z0-9._%+-]+@sitare\.org)$', email):
            return redirect(url_for('teacher_portal'))
        elif re.match(r'^kronit747@gmail\.com$', email):
            return redirect(url_for('admin_portal'))
        else:
            flash("Sorry! Login with a valid Sitare Email id", 'error')
...
def student_portal():
    ...
    for course_id, course_name, instructor_name, instructor_email in course_data:
        ...
        instructor_emails[str(course_id)] = instructor_email
...
def submit_all_forms():
    ...
    student_email_id = user_info['email']
    ...
    for key, values in data.items():
        ...
        if match:
            ...
            field = match.group(2)
            if field in ['understanding', 'revision', 'suggestion']:
    ...
    current_week = (date_of_feedback - datetime(2024, 8, 27).date()).days // 7 + 1
        ...
        with conn.cursor() as cursor:
            for course_id, form_data in feedback_entries.items():
                cursor.execute("""
                    ...
                    (course_id, student_email, student_name, date_of_feedback, week, question1_rating, question2_rating, remarks)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (course_id, student_email, date_of_feedback) DO UPDATE 
                    SET question1_rating = EXCLUDED.question1_rating,
                        question2_rating = EXCLUDED.question2_rating,
                ...
                """, (
                    ...
                    date_of_feedback, 
                    current_week, 
                    form_data['understanding'], 
                    form_data['revision'], 
                    form_data.get('suggestion', '')
                ))
            conn.commit()
...
def teacher_portal():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT c.course_id, c.course_name, f.date_of_feedback, f.week, 
                       f.question1_rating, f.question2_rating, f.remarks, f.student_email
                FROM courses c
                JOIN instructors i ON c.instructor_id = i.instructor_id
                LEFT JOIN feedback f ON c.course_id = f.course_id
                WHERE i.instructor_email = %s AND f.date_of_feedback >= CURRENT_DATE - INTERVAL '2 weeks'
                ORDER BY c.course_id, f.week, f.date_of_feedback DESC
            """, (instructor_email,))
            feedback_data = cursor.fetchall()

        feedback_by_course = defaultdict(lambda: {'course_name': '', 'data': []})
        for row in feedback_data:
            course_id, course_name = row[0], row[1]
            feedback_by_course[course_id]['course_name'] = course_name
            feedback_by_course[course_id]['data'].append(row[2:])
        ...
        for course_id, course_info in feedback_by_course.items():
            ...
            latest_date = max(row[0] for row in course_data) if course_data else None
...
def calculate_average_ratings_by_week(feedback_data):
    ...
    for row in feedback_data:
        week = row[1]  # Assuming Week is the second column
        q1_rating = row[2] if row[2] is not None else 0
        q2_rating = row[3] if row[3] is not None else 0
    ...
    for week, ratings in weekly_ratings.items():
        avg_q1 = ratings['q1_total'] / ratings['count'] if ratings['count'] > 0 else 0
        avg_q2 = ratings['q2_total'] / ratings['count'] if ratings['count'] > 0 else 0
...
def admin_portal():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT i.instructor_email, i.instructor_name, c.course_name, 
                       f.date_of_feedback, f.week, f.question1_rating, f.question2_rating, f.remarks
                FROM instructors i
                JOIN courses c ON i.instructor_id = c.instructor_id
                LEFT JOIN feedback f ON c.course_id = f.course_id
                WHERE f.date_of_feedback >= CURRENT_DATE - INTERVAL '2 weeks'
                ORDER BY i.instructor_email, c.course_name, f.week, f.date_of_feedback DESC
            """)
            feedback_data = cursor.fetchall()

        feedback_by_instructor = defaultdict(lambda: defaultdict(list))
        for row in feedback_data:
            email, name, course, date, week, q1, q2, remarks = row
            feedback_by_instructor[email]['name'] = name
            feedback_by_instructor[email]['courses'][course].append((date, week, q1, q2, remarks))

        # Calculate average ratings for each instructor
        avg_ratings_by_instructor = {}
        for email, data in feedback_by_instructor.items():
            total_q1 = sum(q1 for course in data['courses'].values() for _, _, q1, _, _ in course if q1 is not None)
            total_q2 = sum(q2 for course in data['courses'].values() for _, _, _, q2, _ in course if q2 is not None)
            total_feedback = sum(len(course) for course in data['courses'].values())
            avg_q1 = total_q1 / total_feedback if total_feedback > 0 else 0
            avg_q2 = total_q2 / total_feedback if total_feedback > 0 else 0
            avg_ratings_by_instructor[email] = (avg_q1, avg_q2)

        return render_template(
            'admin_portal.html',
            user_info=user_info,
            feedback_by_instructor=feedback_by_instructor,
            avg_ratings_by_instructor=avg_ratings_by_instructor
        )
    except psycopg2.Error as e:
        print("Database error:", str(e))
        return "An error occurred while fetching data", 500
...
def send_email():
    ...
    recipients = os.getenv('REMINDER_RECIPIENTS', '').split(',')
    subject = "Weekly Feedback Reminder"
    body = "This is your weekly reminder to submit feedback for your courses."
    ...
    try:
        ...
        print("Error sending reminder email:", str(e))
