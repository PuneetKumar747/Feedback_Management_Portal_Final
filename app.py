...
def student_portal():
    ...
    is_feedback_allowed = (current_day == 6 or current_day == 5 or current_day == 4)  # 6 for sunday,5 for saturday,4 for friday 
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM feedback WHERE studentemaiid = %s AND dateOffeedback = %s AND active = TRUE", (student_email_id, current_date))
                ...
                if conn:
                    with conn.cursor() as cursor:
                        query = """
                            ...
                            WHERE 
                                f.studentemaiid = %s AND f.active = TRUE
                        """
...
def get_feedback_data(instructor_email):
    query = """
        ...
        WHERE f.instructorEmailID = %s AND f.active = TRUE
...
=======
    ...
    (17, 'Dr. Aniket Prabhune', 'aniket.prabhune@sitare.org'),
    (18, 'Dr. Sumeet Agrawal', 'sumeet@sitare.org')
>>>>>>> 0e5b3eef703f42d95c39efd7f6b1b764f0362d4d
